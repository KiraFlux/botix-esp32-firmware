...
private:
    ...
    static constexpr kf::Timer::Config update_timer_config{.value = 100}, timeout_timer_config{.value = 1000};

    /// @brief Timer that fires at 100 Hz to write the latest setpoints to the motor drivers
    kf::Timer _update_timer{update_timer_config};
    ...
    void pollImpl(kf::units::Milliseconds now) noexcept {
        ...
        if (_update_timer.expired(now)) {
            ...
            _periphery.servo_claw.set(kf::math::linearMap<kf::units::Degrees>(
                _servo_claw_set,
                0, Input::max_value,
                _periphery.config.servo.angle_range.start,
                _periphery.config.servo.angle_range.end));

            _periphery.servo_arm.set(kf::math::linearMap<kf::units::Degrees>(
                _servo_arm_set,
                0, Input::max_value,
                _periphery.config.servo.angle_range.start,
                _periphery.config.servo.angle_range.end));
